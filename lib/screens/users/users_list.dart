...
class UsersScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<ItemProvider>(
      builder: (context, provider, child) {
        return Scaffold(
          appBar: AppBar(
            title: const Text('Usuários / Instrutores', style: TextStyle(color: Colors.white),),
            backgroundColor: SimpTheme.azul,   // ← Cor correta
          ),
  ...
  void _editUser(BuildContext context, User user) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Editar ${user.nome} - em breve')),
    );
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              await Provider.of<ItemProvider>(context, listen: false).deletarUser(id);
